...
public ArrayList<Movimiento> generaPosiblesMovimientos(int [][] mundo, int turno){
  ArrayList<Movimiento> configuraciones = new ArrayList<Movimiento>();
  ...
  for(int i = 0; i<ancho; i++){
    for(int j = 0; j<alto; j++){
      ...
      if(mundo[j][i] == 0 && !casillasACambiar(i,j, mundo,turno).isEmpty()){
        ...
        Movimiento posibleConfiguracion = new Movimiento(i,j,posibleMundo);
...
void mouseClicked() {
    if (tablero.mundo[mouseY/tamano][mouseX/tamano] == 0 && tiroValido(casillasACambiar(mouseX/tamano,mouseY/tamano, tablero.mundo, jugadorActual()))) { //Si la casilla está vacía entonces...
      ...
      Movimiento proximaConfiguracion = getMovimiento();
